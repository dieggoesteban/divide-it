...
- [ ] T001 Verify project builds and runs locally
...
- [ ] T002 Update `Participant` and `Item` interfaces in `src/core/participants.ts`
- [ ] T003 Update `ParticipantsContext` reducer to handle `items` initialization in `src/context/ParticipantsContext.tsx`
- [ ] T004 Update `getTotal` function to respect items sum in `src/core/participants.ts`
...
- [ ] T005 [US1] Create `ItemManagerDialog` component for managing items in `src/components/ItemManagerDialog.tsx`
- [ ] T006 [US1] Add validation for item fields (description required, amount > 0) in `src/components/ItemManagerDialog.tsx`
- [ ] T007 [US1] Integrate `ItemManagerDialog` into `AddParticipantForm` in `src/components/AddParticipantForm.tsx`
- [ ] T008 [US1] Implement auto-calculation of total from items in `src/components/AddParticipantForm.tsx`
...
- [ ] T009 [US2] Update `ParticipantDetailsPage` to render items list in `src/pages/ParticipantDetailsPage.tsx`
- [ ] T010 [US2] Add items indicator (badge/icon) to participant list items in `src/components/ParticipantsList.tsx`
...
- [ ] T011 [US3] Add "Edit Items" button using `ItemManagerDialog` in `src/pages/ParticipantDetailsPage.tsx`
- [ ] T012 [US3] Ensure context updates correctly on item edit in `src/context/ParticipantsContext.tsx`

## Phase 6: Polish & Cross-Cutting Concerns
*Goal: Ensure UX requirements and edge cases are handled.*

- [ ] T013 Enforce 2 decimal precision in display in `src/components/ItemManagerDialog.tsx` and `src/pages/ParticipantDetailsPage.tsx`
- [ ] T014 Truncate long descriptions in `src/pages/ParticipantDetailsPage.tsx`
- [ ] T015 Verify negative amounts are blocked in `src/components/ItemManagerDialog.tsx`
